<?php
    ...
    $b_id = $_GET['id'];
    $query = "select * from board where b_id ='$b_id'";
    $ret = mysqli_query($conn, $query);
    ...
    $query = "select * from board where b_id < $b_id order by b_id desc limit 1";
    $pp = mysqli_query($conn, $query);
    if (mysqli_num_rows($pp) == 0) {
        $prev_link = '#';
    } else {
        $prev = mysqli_fetch_array($pp);
        $prev_link = 'board_detail?id=' . $prev['b_id'];
        $prev_title = $prev['title'];
    }
    $query2 = "select * from board where b_id > $b_id order by b_id limit 1";
    $nn = mysqli_query($conn, $query2);
    if (mysqli_num_rows($nn) == 0) {
        $next_link = '#';
    } else {
        $next = mysqli_fetch_array($nn);
        $next_link = 'board_detail?id=' . $next['b_id'];
    ...
    <section id="pre">
        ...
        <div class="notice_detail_area">
            ...
            <div class="notice_detail_comment_area">
                ...
                <form id = "reply_form" method = "POST" action = "form/board_reply">
                    <div class="notice_detail_comment_insert_area">
                        <div class="notice_detail_comment_bg">
                            <div class="notice_detail_comment_insert_name">
                                <div>작성자</div>
                                <input type="text" class="input_name" id = "writer" name = "writer" value ="<?php echo $user['u_id']?>">
                ...
                <div class="notice_detail_btn_list">
                    <?php
                        if ($notice['u_code'] == $u_code) {
                            echo "<div class=\"notice_detail_mod_btn\" style = 'cursor:pointer' onclick=\"location.href='board_modify?id=$b_id'\">
                                수정
                            </div>";
                        }
                    ?>
                    <!--<div class="notice_detail_del_btn" onclick="">
                        삭제
                    </div> -->
                    <div class="notice_detail_list_btn" style ="cursor:pointer" onclick="location.href='board'">
                        목록
                    </div>
                </div>
...
function sendLink() {
  ...
  Kakao.Link.sendScrap({
    requestUrl: 'http://kmm.or.kr/board_detail?id='+id
  });
}
</script>
</html>